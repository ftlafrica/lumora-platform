...
const culturalQualityOperations = {
...
const searchRetrievalOperations = {
...
const assetOpsOperations = {
  summary: { trackedAssets: 284, criticalAssets: 42, ownerCoverage: "96%", lifecycleRisks: 9, dependencyCoverage: "88%" },
  serviceInventory: [
    { service: "Chat gateway", tier: "Tier 0", owner: "Platform", dependency: "Model gateway", status: "Critical" },
    { service: "Model gateway", tier: "Tier 0", owner: "AI Ops", dependency: "Provider tokens", status: "Critical" },
    { service: "Billing ledger", tier: "Tier 1", owner: "Revenue Ops", dependency: "Payment processor", status: "Healthy" },
    { service: "Admin console", tier: "Tier 1", owner: "Enterprise", dependency: "Seed admin access", status: "Protected" }
  ],
  dataAssets: [
    { asset: "User profile store", class: "Restricted", region: "Regional", owner: "Data Platform", status: "Protected" },
    { asset: "Conversation metadata", class: "Confidential", region: "Regional", owner: "Chat Platform", status: "Healthy" },
    { asset: "Language correction samples", class: "Controlled", region: "Pan-African", owner: "Language QA", status: "Consent gated" },
    { asset: "Billing events", class: "Restricted", region: "Global", owner: "Finance Data", status: "Certified" }
  ],
  ownershipGaps: [
    { gap: "Legacy prompt templates", surface: "Creator Studio", owner: "Product", severity: "Medium", status: "Assigning" },
    { gap: "Partner webhook sandbox", surface: "Integrations", owner: "API Platform", severity: "Low", status: "Review" },
    { gap: "Regional launch checklist archive", surface: "Regional Ops", owner: "Launch", severity: "Low", status: "Mapped" },
    { gap: "Voice beta benchmark scripts", surface: "Voice Ops", owner: "AI QA", severity: "Medium", status: "Review" }
  ],
  dependencyMap: [
    { dependency: "Payment processor", consumers: "Billing, plans, entitlements", criticality: "High", owner: "Revenue Ops", status: "Healthy" },
    { dependency: "Hugging Face model sources", consumers: "AI gateway, evals", criticality: "High", owner: "AI Ops", status: "Watch" },
    { dependency: "Push notification provider", consumers: "Mobile, communications", criticality: "Medium", owner: "Mobile Ops", status: "Healthy" },
    { dependency: "Object storage", consumers: "Uploads, multimodal, evidence", criticality: "High", owner: "Infrastructure", status: "Protected" }
  ],
  lifecycleQueue: [
    { asset: "Old pricing experiment flags", action: "Archive", owner: "Growth", due: "Aug 16", status: "Queued" },
    { asset: "Deprecated model route v0", action: "Retire", owner: "AI Ops", due: "Aug 20", status: "Review" },
    { asset: "Legacy admin export CSV", action: "Restrict", owner: "Security", due: "Aug 14", status: "Urgent" },
    { asset: "Mobile beta artifact set", action: "Retain", owner: "Mobile Ops", due: "Per release", status: "Active" }
  ],
  guardrails: [
    "Every service, data store, model route, integration, report, and admin tool needs an accountable owner and lifecycle state.",
    "Critical asset inventory should link dependencies, backup posture, access controls, incidents, and change history.",
    "Assets without owners should not be used for production decisions, compliance evidence, or customer commitments.",
    "Asset views must expose metadata and ownership without leaking raw prompts, files, payment secrets, or private customer data."
  ]
};
...
const complianceAutomationOpsOperations = {
...
const accessReviewOpsOperations = {
...
const sessionRiskOpsOperations = {
...
const threatIntelOpsOperations = {
...
const socOpsOperations = {
  summary: { alertsToday: 1264, highPriorityOpen: 14, meanTriage: "6m", automationRate: "72%", analystsOnShift: 8 },
  alertQueue: [
    { alert: "Privileged login anomaly", source: "Identity", severity: "High", owner: "SOC West", status: "Investigating" },
    { alert: "Prompt extraction pattern", source: "AI gateway", severity: "High", owner: "AI Safety", status: "Contained" },
    { alert: "Payment webhook replay", source: "Billing", severity: "Critical", owner: "SOC East", status: "Escalated" },
    { alert: "Bulk workspace download", source: "DLP", severity: "Medium", owner: "Privacy", status: "Triage" }
  ],
  detectionCoverage: [
    { domain: "Identity and admin", techniques: "24/27", telemetry: "98%", owner: "Identity", status: "Healthy" },
    { domain: "AI and model routes", techniques: "18/24", telemetry: "91%", owner: "AI Safety", status: "Improving" },
    { domain: "Payments and subscriptions", techniques: "16/18", telemetry: "97%", owner: "Revenue Ops", status: "Healthy" },
    { domain: "Mobile and device", techniques: "12/19", telemetry: "84%", owner: "Mobile", status: "Watch" }
  ],
  activeCases: [
    { case: "SOC-2481", focus: "Credential stuffing cluster", age: "43m", owner: "Trust", status: "Contained" },
    { case: "SOC-2478", focus: "Reviewer account takeover", age: "2h", owner: "Security", status: "Investigating" },
    { case: "SOC-2472", focus: "API token enumeration", age: "5h", owner: "DevEx", status: "Monitoring" },
    { case: "SOC-2469", focus: "Suspicious model scraping", age: "9h", owner: "AI Ops", status: "Escalated" }
  ],
  automationPlaybooks: [
    { playbook: "Revoke risky session", trigger: "Identity risk > 85", executions: 38, owner: "Identity", status: "Active" },
    { playbook: "Throttle hostile API source", trigger: "Abuse burst detected", executions: 112, owner: "DevEx", status: "Active" },
    { playbook: "Quarantine sensitive export", trigger: "DLP high confidence", executions: 19, owner: "Privacy", status: "Active" },
    { playbook: "Freeze payment instrument", trigger: "Card testing cluster", executions: 7, owner: "Fraud Ops", status: "Approval required" }
  ],
  analystWorkload: [
    { shift: "West Africa", analysts: 3, openAlerts: 18, handoff: "14:00 WAT", status: "Balanced" },
    { shift: "East Africa", analysts: 2, openAlerts: 15, handoff: "17:00 EAT", status: "Busy" },
    { shift: "Southern Africa", analysts: 2, openAlerts: 9, handoff: "16:00 SAST", status: "Balanced" },
    { shift: "Global on-call", analysts: 1, openAlerts: 6, handoff: "Follow-the-sun", status: "Ready" }
  ],
  guardrails: [
    "SOC triage must combine identity, web, mobile, API, payments, DLP, model, voice, and reviewer-network telemetry in one auditable workflow.",
    "Automation may contain clear threats, but destructive account, payment, tenant, or data actions require scoped approval and a reversible path.",
    "Alerts need severity, evidence, owner, SLA, containment state, and escalation criteria before they can be closed.",
    "SOC views must protect investigation details and never expose raw prompts, credentials, private files, payment data, or exploit instructions."
  ]
};

const vulnerabilityOpsOperations = {
...
const securityValidationOpsOperations = {
...
const securityArchitectureOpsOperations = {
...
const insiderRiskOpsOperations = {
...
const securityAwarenessOpsOperations = {
...
function adminCulturalQualityOperations() {
...
function adminVoiceSpeechOperations() {
  return voiceSpeechOperations;
}

function adminTranslationOperations() {
...
function adminSearchRetrievalOperations() {
...
function adminWebOpsOperations() {
...
function adminStatusOpsOperations() {
...
function adminAuditOpsOperations() {
  return auditOpsOperations;
}

function adminChangeOpsOperations() {
  return changeOpsOperations;
}

function adminBackupOpsOperations() {
  return backupOpsOperations;
}

function adminAssetOpsOperations() {
...
function adminCostOpsOperations() {
  return costOpsOperations;
}
...
function adminRegulatoryOpsOperations() {
  return regulatoryOpsOperations;
}

function adminComplianceAutomationOpsOperations() {
...
function adminAccessReviewOpsOperations() {
  return accessReviewOpsOperations;
}

function adminSessionRiskOpsOperations() {
  return sessionRiskOpsOperations;
}

function adminThreatIntelOpsOperations() {
  return threatIntelOpsOperations;
}

function adminSocOpsOperations() {
  return socOpsOperations;
}

function adminVulnerabilityOpsOperations() {
  return vulnerabilityOpsOperations;
}

function adminSecurityValidationOpsOperations() {
  return securityValidationOpsOperations;
}

function adminSecurityArchitectureOpsOperations() {
  return securityArchitectureOpsOperations;
}

function adminInsiderRiskOpsOperations() {
  return insiderRiskOpsOperations;
}

function adminSecurityAwarenessOpsOperations() {
...
function adminSecretsOpsOperations() {
  return secretsOpsOperations;
}
...
async function handler(request, response) {
  ...
  try {
    ...
    if (request.method === "GET" && url.pathname === "/v1/admin/cultural-quality") {
      if (request.headers["x-seed-admin-code"] !== SEED_ADMIN_CODE) {
        return sendJson(response, 403, { error: "Seed admin access required" });
      }
      recordAdminEvent("cultural_quality_viewed", "Cultural Quality", "Info", "Language QA");
    ...
    if (request.method === "GET" && url.pathname === "/v1/admin/voice-speech") {
      if (request.headers["x-seed-admin-code"] !== SEED_ADMIN_CODE) {
        return sendJson(response, 403, { error: "Seed admin access required" });
      }
      recordAdminEvent("voice_speech_viewed", "Voice Ops", "Info", "Voice Ops");
      return sendJson(response, 200, adminVoiceSpeechOperations());
    }

    if (request.method === "GET" && url.pathname === "/v1/admin/translation-ops") {
      if (request.headers["x-seed-admin-code"] !== SEED_ADMIN_CODE) {
        return sendJson(response, 403, { error: "Seed admin access required" });
      }
      recordAdminEvent("translation_ops_viewed", "Translate Ops", "Info", "Language QA");
    ...
    if (request.method === "GET" && url.pathname === "/v1/admin/search-retrieval") {
      if (request.headers["x-seed-admin-code"] !== SEED_ADMIN_CODE) {
        return sendJson(response, 403, { error: "Seed admin access required" });
      }
      recordAdminEvent("search_retrieval_viewed", "Search Ops", "Info", "AI Ops");
    ...
    if (request.method === "GET" && url.pathname === "/v1/admin/web-ops") {
      if (request.headers["x-seed-admin-code"] !== SEED_ADMIN_CODE) {
        return sendJson(response, 403, { error: "Seed admin access required" });
      }
      recordAdminEvent("web_ops_viewed", "Web Ops", "Info", "Web Operations");
    ...
    if (request.method === "GET" && url.pathname === "/v1/admin/status-ops") {
      if (request.headers["x-seed-admin-code"] !== SEED_ADMIN_CODE) {
        return sendJson(response, 403, { error: "Seed admin access required" });
      }
      recordAdminEvent("status_ops_viewed", "Status Ops", "Info", "SRE");
    ...
    if (request.method === "GET" && url.pathname === "/v1/admin/audit-ops") {
      if (request.headers["x-seed-admin-code"] !== SEED_ADMIN_CODE) {
        return sendJson(response, 403, { error: "Seed admin access required" });
      }
      recordAdminEvent("audit_ops_viewed", "Audit Ops", "High", "Security");
      return sendJson(response, 200, adminAuditOpsOperations());
    }

    if (request.method === "GET" && url.pathname === "/v1/admin/change-ops") {
      if (request.headers["x-seed-admin-code"] !== SEED_ADMIN_CODE) {
        return sendJson(response, 403, { error: "Seed admin access required" });
      }
      recordAdminEvent("change_ops_viewed", "Change Ops", "Medium", "Release Management");
      return sendJson(response, 200, adminChangeOpsOperations());
    }

    if (request.method === "GET" && url.pathname === "/v1/admin/backup-ops") {
      if (request.headers["x-seed-admin-code"] !== SEED_ADMIN_CODE) {
        return sendJson(response, 403, { error: "Seed admin access required" });
      }
      recordAdminEvent("backup_ops_viewed", "Backup Ops", "High", "SRE");
      return sendJson(response, 200, adminBackupOpsOperations());
    }

    if (request.method === "GET" && url.pathname === "/v1/admin/asset-ops") {
      if (request.headers["x-seed-admin-code"] !== SEED_ADMIN_CODE) {
        return sendJson(response, 403, { error: "Seed admin access required" });
      }
      recordAdminEvent("asset_ops_viewed", "Asset Ops", "Medium", "Operations");
    ...
    if (request.method === "GET" && url.pathname === "/v1/admin/cost-ops") {
      if (request.headers["x-seed-admin-code"] !== SEED_ADMIN_CODE) {
        return sendJson(response, 403, { error: "Seed admin access required" });
      }
      recordAdminEvent("cost_ops_viewed", "Cost Ops", "High", "Finance/AI Ops");
      return sendJson(response, 200, adminCostOpsOperations());
    }
    ...
    if (request.method === "GET" && url.pathname === "/v1/admin/data-quality") {
      ...
      recordAdminEvent("data_quality_ops_viewed", "Data Quality", "Info", "Data Platform");
      return sendJson(response, 200, adminDataQualityOpsOperations());
    }

    if (request.method === "GET" && url.pathname === "/v1/admin/regulatory-ops") {
      if (request.headers["x-seed-admin-code"] !== SEED_ADMIN_CODE) {
        return sendJson(response, 403, { error: "Seed admin access required" });
      }
      recordAdminEvent("regulatory_ops_viewed", "Regulatory Ops", "High", "Legal/Privacy");
      return sendJson(response, 200, adminRegulatoryOpsOperations());
    }

    if (request.method === "GET" && url.pathname === "/v1/admin/compliance-automation") {
      if (request.headers["x-seed-admin-code"] !== SEED_ADMIN_CODE) {
        return sendJson(response, 403, { error: "Seed admin access required" });
      }
      recordAdminEvent("compliance_automation_viewed", "Compliance Automation", "High", "Security/Legal");
    ...
    if (request.method === "GET" && url.pathname === "/v1/admin/access-reviews") {
      if (request.headers["x-seed-admin-code"] !== SEED_ADMIN_CODE) {
        return sendJson(response, 403, { error: "Seed admin access required" });
      }
      recordAdminEvent("access_reviews_viewed", "Access Reviews", "High", "Security");
      return sendJson(response, 200, adminAccessReviewOpsOperations());
    }

    if (request.method === "GET" && url.pathname === "/v1/admin/session-risk") {
      if (request.headers["x-seed-admin-code"] !== SEED_ADMIN_CODE) {
        return sendJson(response, 403, { error: "Seed admin access required" });
      }
      recordAdminEvent("session_risk_viewed", "Session Risk", "High", "Identity");
      return sendJson(response, 200, adminSessionRiskOpsOperations());
    }

    if (request.method === "GET" && url.pathname === "/v1/admin/threat-intel") {
      if (request.headers["x-seed-admin-code"] !== SEED_ADMIN_CODE) {
        return sendJson(response, 403, { error: "Seed admin access required" });
      }
      recordAdminEvent("threat_intel_viewed", "Threat Intel", "High", "Security");
      return sendJson(response, 200, adminThreatIntelOpsOperations());
    }

    if (request.method === "GET" && url.pathname === "/v1/admin/soc") {
      if (request.headers["x-seed-admin-code"] !== SEED_ADMIN_CODE) {
        return sendJson(response, 403, { error: "Seed admin access required" });
      }
      recordAdminEvent("soc_ops_viewed", "SOC Ops", "Critical", "Security");
      return sendJson(response, 200, adminSocOpsOperations());
    }

    if (request.method === "GET" && url.pathname === "/v1/admin/vulnerabilities") {
      if (request.headers["x-seed-admin-code"] !== SEED_ADMIN_CODE) {
        return sendJson(response, 403, { error: "Seed admin access required" });
      }
      recordAdminEvent("vulnerability_ops_viewed", "Vulnerability Ops", "High", "Security/DevEx");
      return sendJson(response, 200, adminVulnerabilityOpsOperations());
    }

    if (request.method === "GET" && url.pathname === "/v1/admin/security-validation") {
      if (request.headers["x-seed-admin-code"] !== SEED_ADMIN_CODE) {
        return sendJson(response, 403, { error: "Seed admin access required" });
      }
      recordAdminEvent("security_validation_viewed", "Security Validation", "High", "Security/QA");
      return sendJson(response, 200, adminSecurityValidationOpsOperations());
    }

    if (request.method === "GET" && url.pathname === "/v1/admin/security-architecture") {
      if (request.headers["x-seed-admin-code"] !== SEED_ADMIN_CODE) {
        return sendJson(response, 403, { error: "Seed admin access required" });
      }
      recordAdminEvent("security_architecture_viewed", "Security Architecture", "High", "Security/Platform");
      return sendJson(response, 200, adminSecurityArchitectureOpsOperations());
    }

    if (request.method === "GET" && url.pathname === "/v1/admin/insider-risk") {
      if (request.headers["x-seed-admin-code"] !== SEED_ADMIN_CODE) {
        return sendJson(response, 403, { error: "Seed admin access required" });
      }
      recordAdminEvent("insider_risk_viewed", "Insider Risk", "Critical", "Security/Privacy");
      return sendJson(response, 200, adminInsiderRiskOpsOperations());
    }

    if (request.method === "GET" && url.pathname === "/v1/admin/security-awareness") {
      if (request.headers["x-seed-admin-code"] !== SEED_ADMIN_CODE) {
        return sendJson(response, 403, { error: "Seed admin access required" });
      }
      recordAdminEvent("security_awareness_viewed", "Security Awareness", "Medium", "Security/People");
    ...
    if (request.method === "GET" && url.pathname === "/v1/admin/dlp") {
      ...
      recordAdminEvent("dlp_ops_viewed", "DLP Ops", "High", "Security/Privacy");
      return sendJson(response, 200, adminDlpOpsOperations());
    }
    ...
    if (request.method === "GET" && url.pathname === "/v1/admin/consent") {
      ...
      recordAdminEvent("consent_ops_viewed", "Consent Ops", "High", "Privacy");
      return sendJson(response, 200, adminConsentOpsOperations());
    }
    ...
    if (request.method === "GET" && url.pathname === "/v1/admin/secrets") {
      ...
      recordAdminEvent("secrets_ops_viewed", "Secrets Ops", "High", "Security");
      return sendJson(response, 200, adminSecretsOpsOperations());
    }
